...
class LocationTimelineViewTest(unittest.TestCase):
    ...
    def test_location_weather_chart_renders_daily_temperature_range_area(self):
        ...
        self.assertIn('const padding = { top: 24, right: isCompactChart ? 168 : 228, bottom: 38, left: 52 };', html)
        self.assertIn('const rightAxisGap = isCompactChart ? 54 : 72;', html)
        self.assertIn('x: cssWidth - padding.right + (rightAxisGap * 2),', html)
        self.assertIn('const maxTickLabelWidth = tickLabels.reduce((width, label) => Math.max(width, context.measureText(label).width), 0);', html)
        self.assertIn('maxTitleX: rightAxisTitleMaxX,', html)
